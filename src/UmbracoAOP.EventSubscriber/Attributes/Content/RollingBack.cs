...
namespace UmbracoAOP.EventSubscriber.Attributes.ContentEvents
{
    public static partial class ContentEvent
    {
        ...
        public class RollingBack : Attribute, IBindToEvent
        {
            /// <summary>
            ...
            public RollingBack() { ContentTypeAliases = new string[] {}; }

            /// <summary>
            /// Filter by Content Type Aliases
            /// </summary>
            /// <param name="contentTypeAlias"></param>
            public RollingBack(params string[] contentTypeAliases)
            {
            ...
            public void Bind(MethodInfo methodToBind)
            {
                ...
                if (ContentTypeAliases.Length > 0)
                {
                    ...
                    ContentService.RollingBack += FilterEvent;
                }
                else
                {
                    //bind without filter
                    var eventBinder = new EventBinder();
                    eventBinder.BindToEvent(typeof(ContentService), "RollingBack", methodToBind);
                }
            }
