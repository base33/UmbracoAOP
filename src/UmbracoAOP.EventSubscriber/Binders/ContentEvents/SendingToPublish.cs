...
namespace UmbracoAOP.EventSubscriber.Attributes.ContentEvents
{
    public static partial class ContentEvent
    {
        ...
        public class SendingToPublish : Attribute, IBindToEvent
        {
            /// <summary>
            ...
            public SendingToPublish() { ContentTypeAliases = new string[] {}; }

            /// <summary>
            /// Filter by Content Type Aliases
            /// </summary>
            /// <param name="contentTypeAlias"></param>
            public SendingToPublish(params string[] contentTypeAliases)
            {
            ...
            public void Bind(MethodInfo methodToBind)
            {
                ...
                if (ContentTypeAliases.Length > 0)
                {
                    ...
                    ContentService.SendingToPublish += FilterEvent;
                }
                else
                {
                    //bind without filter
                    var eventBinder = new EventBinder();
                    eventBinder.BindToEvent(typeof(ContentService), "SendingToPublish", methodToBind);
                }
            }
